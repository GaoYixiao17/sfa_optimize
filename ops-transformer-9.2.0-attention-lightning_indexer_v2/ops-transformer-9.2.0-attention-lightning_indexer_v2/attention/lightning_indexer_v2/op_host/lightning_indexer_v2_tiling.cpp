...
ge::graphStatus LightningIndexerV2Tiling::DoTiling(LIV2TilingInfo *tilingInfo)
{
    ...
    if (ascendcPlatform.GetCurNpuArch() == NpuArch::DAV_3510) {
        // 对齐 kernel 侧 Init 的 workspace 偏移计算(lightning_indexer_v2_kernel_arch35.h Init):
        //   [score 区:  blockDim * s1BaseSize * Align(S2,128) * 4B]
        //   [ldScore 区: blockDim * s1BaseSize * Align(topk,16) * 2(头/尾) * 4B]
        //   [ldIndex 区: blockDim * s1BaseSize * Align(topk,16) * 2(头/尾) * 4B]
        // s1BaseSize 与 kernel InitTilingData 一致: gSize>32 或 topk>2048 时为 2, 否则为 4。
        // 原公式沿用了 arch22 分支的 S1_BASE_SIZE=8/TOPK_MAX_SIZE=8192 常量, 在小 topk 场景
        // 过度申请(topk=64 时 24.6MB → ~0.2MB); workspaceIdx 槽位数(blockDim*2)与
        // kernel 寻址上限保持一致, 不改变 metadata 调度契约。
        constexpr uint32_t li3510S1Base = 4;
        constexpr uint32_t li3510S1BaseSmall = 2;
        constexpr uint32_t li3510S2Base = 128;
        constexpr uint32_t li3510TopkAlign = 16;
        constexpr uint32_t li3510LdPieceNum = 2; // 头/尾两块LD数据
        uint32_t s1BaseSize = (tilingInfo->gSize > 32 || tilingInfo->topk > 2048) ? li3510S1BaseSmall : li3510S1Base;
        uint64_t s2SizeAlign =
            (static_cast<uint64_t>(tilingInfo->s2Size) + li3510S2Base - 1) / li3510S2Base * li3510S2Base;
        uint64_t topkAlign =
            (static_cast<uint64_t>(tilingInfo->topk) + li3510TopkAlign - 1) / li3510TopkAlign * li3510TopkAlign;
        uint64_t blockNum = static_cast<uint64_t>(blockDim);
        workspaceSize += blockNum * s1BaseSize * s2SizeAlign * sizeof(uint32_t);
        workspaceSize += 2 * li3510LdPieceNum * blockNum * s1BaseSize * topkAlign * sizeof(uint32_t);
    } else {
        // 主流程需Workspace大小
