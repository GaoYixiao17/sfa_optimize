...
protected:
    ...
    // ============ K 数据跨 base block 缓存 ============
    // S2<=s2BaseSize 时, 同一 (b,n2) 的所有 gS1 块使用完全相同的 K(g=64 时 64 块重复搬运):
    // KeyNd2Nz(GM→L1 32KB) 与 LoadKeyToL0b(L1→L0 32KB) 对同一 (tensorKeyOffset, s2块) 重复执行。
    // L1 K 槽 3 轮换; L0 K 槽随 l0BufIdx_ 2 轮换。 按 (tensorKeyOffset, s2GmBaseOffset) 记录槽内容,
    // 命中则跳过搬运; 槽级 KEY_MTE1_MTE2 / M_MTE1 事件照常 Set/Wait, 仅跳过搬运本体与
    // 块内自包含的 MTE2_MTE1 / MTE1_M 配对屏障(无搬运则无需屏障)。 未写槽保持旧标签, 与内容一致。
...
__aicore__ inline void LightningIndexerV2ServiceCube<LIT>::ComputeMm1(const LIV2Common::RunInfo &runInfo)
{
    ...
    for (uint64_t s2GmOffset = 0; s2GmOffset < s2ProcessSize; s2GmOffset += S2_BASIC_BLOCK) {
        ...
        // K 跨块缓存: tag 含 s2GmOffset(一个块内多个 s2 chunk 时各 chunk 的 K 不同)。
        // L1 槽按 keyL1BufIdx_ 轮换(chunk 粒度); L0 槽按 l0BufIdx_ 轮换(每个 L0 迭代)。
        ...
        for (uint64_t s1gGmOffset = 0; s1gGmOffset < s1gProcessSize; s1gGmOffset += M_BASIC_BLOCK) {
            ...
            for (uint64_t s2L1Offset = 0; s2L1Offset < s2L1RealSize; s2L1Offset += S2_BASIC_BLOCK_L0) {
                ...
                for (uint64_t s1gL1Offset = 0; s1gL1Offset < s1gL1SizeAlign; s1gL1Offset += M_BASIC_BLOCK) {
                    ...
                    // L0 K 槽命中: 同 (b,n2) 上一块已装载相同 K(chunk 相同), 跳过 L1→L0 搬运
                    uint64_t kl0Slot = l0BufIdx_ % L0_BUF_NUM;
