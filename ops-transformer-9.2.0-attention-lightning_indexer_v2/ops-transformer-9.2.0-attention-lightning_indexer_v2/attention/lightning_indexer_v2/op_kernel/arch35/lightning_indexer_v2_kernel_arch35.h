...
{
    if ASCEND_IS_AIV {
        // S2 <= s2BaseSize(128) 时, 每个 (b,n2,gS1) 行组至多 1 个 s2 块, 结构上不可能产生
        // LD(跨核 s2 切分)任务 → 跳过 SyncAll 全核屏障与 LD 代码预取。
        // 条件取自 tiling(s2Size), 所有 AIV 判定一致, 不存在部分核跳过导致 SyncAll 悬挂的风险。
        if (constInfo.kSeqSize <= constInfo.s2BaseSize) {
            return;
        }
