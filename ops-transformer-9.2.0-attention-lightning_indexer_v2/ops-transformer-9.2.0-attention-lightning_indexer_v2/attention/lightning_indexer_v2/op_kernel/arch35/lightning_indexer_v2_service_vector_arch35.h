...
private:
    ...
    // ============ 退化路径(validS2Len < topkCount, 非LD)模板化优化 ============
    // 输出 indices = [outputIdxOffset..outputIdxOffset+v) + (-1)填充, 与 score 无关;
    // 相邻 token 的 v 单调 +1 → indicesOutLocal_/valueOutLocal_ 常驻模板, 每 token 只 patch 边界:
    //   - v 增大: CreateVecIndex 增量补 [state, v); v 减小(新 batch): Duplicate(-1/-inf) 补 [v, state)
    //   - 真实 topk / LD 路径会复用这些 buffer 作 scratch, 使用后置 -1 失效
    // state = 模板当前有效前缀长度; -1 表示未初始化。 仅在 !isNeedLD 时维护。
    int32_t degenIdxState_ = -1;
    int32_t degenIdxOffset_ = 0;
    int32_t degenValState_ = -1;

    __aicore__ inline void DegenIdxTemplateInit()
    {
        PipeBarrier<PIPE_V>();
        AscendC::Duplicate(indicesOutLocal_.ReinterpretCast<int32_t>(), (int32_t)-1, topkCount_);
        PipeBarrier<PIPE_V>();
        degenIdxState_ = 0;
    }
    ...
    __aicore__ inline void PatchDegenIdxTemplate(int32_t v, int32_t outputIdxOffset)
    {
        if (degenIdxState_ < 0 || degenIdxOffset_ != outputIdxOffset) {
            DegenIdxTemplateInit();
            degenIdxOffset_ = outputIdxOffset;
        }
        if (v > degenIdxState_) {
            PipeBarrier<PIPE_V>();
    ...
    __aicore__ inline void DegenValTemplateInit()
    {
        PipeBarrier<PIPE_V>();
        AscendC::Duplicate(valueOutLocal_.template ReinterpretCast<uint32_t>(), constInfo_.NEG_INF_FLOAT, topkCount_);
        PipeBarrier<PIPE_V>();
        degenValState_ = 0;
    }

    // 将 values 模板(-inf 后缀)对齐到有效前缀 v; convEnd 为本轮转换写脏的右边界
    __aicore__ inline void PatchDegenValTemplate(int32_t v, int32_t convEnd)
    {
        if (degenValState_ < 0) {
            DegenValTemplateInit();
        }
        int32_t fillEnd = convEnd > degenValState_ ? convEnd : degenValState_;
        if (fillEnd < v) {
            fillEnd = v;
        }
        if (fillEnd > (int32_t)topkCount_) {
            fillEnd = (int32_t)topkCount_;
        }
        if (fillEnd > v) {
            PipeBarrier<PIPE_V>();
...
__aicore__ inline void LightningIndexerV2ServiceVector<LIT>::CleanInvalidOutput(int64_t invalidS1Offset)
{
    ...
    if (returnValueFlag) {
        ...
        // CleanInvalidOutput 将 valueOutLocal_ 刷为全 -inf, 等价于退化模板 v=0 态, 同步状态避免重建
        degenValState_ = 0;
...
__aicore__ inline void LightningIndexerV2ServiceVector<LIT>::ProcessTopK(const LIV2Common::RunInfo &info)
{
    ...
    if (info.isNeedLD) {
        // LD 路径会改写 indicesOutLocal_(Adds/尾插-1), 退化模板与 buffer 内容脱钩, 强制失效;
        // 避免后续非 LD 任务组的退化 token 用陈旧 state patch 出错误结果
        degenIdxState_ = -1;
        degenValState_ = -1;
    }
    ...
    for (uint32_t i = 0; i < curAivS1ProcNum; i++) {
        ...
        if (validS2Len <= 0 && !info.isNeedLD) {
            // 模板化: v=0 → indices 全 -1 / values 全 -inf, patch 模板后直接拷出,
            // 避免每 token 全量 Duplicate(topkCount_)
            WaitFlag<HardEvent::MTE3_V>(TOPK_MTE3_V_EVENT);
            PatchDegenIdxTemplate(0, static_cast<int32_t>(outputIdxOffset));
            if (returnValueFlag) {
                PatchDegenValTemplate(0, 0);
            }
            SetFlag<HardEvent::V_MTE3>(TOPK_V_MTE3_EVENT);
            WaitFlag<HardEvent::V_MTE3>(TOPK_V_MTE3_EVENT);
        ...
        if (validS2Len >= topkCount_) {
            // 真实 topk 路径会复用 indicesOutLocal_(topk scratch)与 valueOutLocal_(转换目标),
            // 退化模板失效, 后续退化 token 需全量重建
            degenIdxState_ = -1;
            degenValState_ = -1;
            ...
            // 退化路径 (v < topkCount, 非LD): indices = [offset..offset+v) + (-1), 与 score 无关;
            // 模板常驻 indicesOutLocal_, 每 token 只 patch 边界(相邻 token v 单调 +1)
            if (!info.isNeedLD) {
                PatchDegenIdxTemplate(validS2Len, static_cast<int32_t>(outputIdxOffset));
            } else {
        ...
        if (!info.isNeedLD) {
            // 退化路径的 -1 / -inf 填充已由模板维护(见 PatchDegenIdxTemplate/PatchDegenValTemplate),
            // 真实 topk 路径 (v >= topkCount_) 无需填充
            ...
            if (returnValueFlag) {
                WaitFlag<HardEvent::V_MTE2>(TOPK_V_MTE2_EVENT);
                if (validS2Len < topkCount_) {
                    // 退化路径: 只转换有效前缀(128对齐), -inf 后缀由模板维护,
                    // 避免按 topkCountAlign256_ 全宽转换(如 topk=2048 时 16 轮 → 1 轮)
                    int32_t convEnd = LIV2Common::Align(validS2Len, (int32_t)128);
                    liV2Vector1::UIntToFloatReturnValue(valueOutLocal_, scoreOutLocal_, static_cast<uint32_t>(convEnd),
                                                        constInfo_.NEG_INF_FLOAT);
                    PatchDegenValTemplate(validS2Len, convEnd);
                } else {
