...
protected:
    ...
    // ============ actual_seq 批量缓存 ============
    // GetS1S2ActualSeqLen 在 GetTotalBaseBlockNum/SplitCore 两遍扫描与主循环中被反复调用
    // (B=64 时每核 ~200+ 次串行 GM 标量读)。 提供 actual_seq 时按 batch 缓存到 local memory,
    // 每 batch 仅首次读 GM; 未提供 actual_seq 时 GetActualSeqLen 走 defaultSeqLen 无 GM 读,
    // 缓存命中/未命中均不影响语义。 超出缓存容量的 batch 走原路径。
    static constexpr uint32_t SEQ_LEN_CACHE_MAX_B = 128;
    static constexpr uint32_t SEQ_LEN_CACHE_INVALID = 0xFFFFFFFFU;
    uint32_t seqLenCacheBIdx_[SEQ_LEN_CACHE_MAX_B];
    uint32_t seqLenCacheS1_[SEQ_LEN_CACHE_MAX_B];
    uint32_t seqLenCacheS2_[SEQ_LEN_CACHE_MAX_B];
    uint32_t seqLenCacheS2Orig_[SEQ_LEN_CACHE_MAX_B];
                                                                     ...
                                                                     __gm__ uint8_t *actualSeqLengthsK)
{
    ...
    for (uint32_t i = 0; i < SEQ_LEN_CACHE_MAX_B; i++) {
        seqLenCacheBIdx_[i] = SEQ_LEN_CACHE_INVALID;
    }
}
...
{
    if (bIdx < SEQ_LEN_CACHE_MAX_B) {
        if (seqLenCacheBIdx_[bIdx] != SEQ_LEN_CACHE_INVALID) {
            actS1Size = seqLenCacheS1_[bIdx];
            actS2Size = seqLenCacheS2_[bIdx];
            actS2SizeOrig = seqLenCacheS2Orig_[bIdx];
            return;
        }
        ...
        seqLenCacheS1_[bIdx] = actS1Size;
        seqLenCacheS2_[bIdx] = actS2Size;
        seqLenCacheS2Orig_[bIdx] = actS2SizeOrig;
        seqLenCacheBIdx_[bIdx] = bIdx;
        return;
    }
    actS1Size = GetActualSeqLen(bIdx, constInfo.actualLenQDims, constInfo.isAccumSeqS1, actualSeqLengthsGmQ,
                                constInfo.qSeqSize);
