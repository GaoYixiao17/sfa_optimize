...
protected:
    ...
    // ============ K 数据跨 base block 缓存 ============
    // S2<=s2BaseSize 时, 同一 batch 的所有 gS1 块使用完全相同的 K:
    // KeyNd2Nz(GM→L1 32KB) 与 LoadKeyToL0b(L1→L0 32KB) 对同一 (batch, s2块) 重复搬运。
    // L1 K 槽 3 轮换 / L0 K 槽 2 轮换, 按 (bIdx, s2GmBaseOffset) 记录槽内容, 命中则跳过搬运,
    // 事件协议不变: 槽级 KEY_MTE1_MTE2/M_MTE1 事件照常 Set/Wait, 仅跳过搬运本体与
    // 块内自包含的 MTE2_MTE1 / MTE1_M 配对屏障(无搬运则无需屏障)。
    static constexpr uint64_t KEY_CACHE_INVALID = 0xFFFFFFFFFFFFFFFFULL;
    uint64_t keyL1CacheBIdx_[KEY_BUF_NUM] = {KEY_CACHE_INVALID, KEY_CACHE_INVALID, KEY_CACHE_INVALID};
    uint64_t keyL1CacheS2_[KEY_BUF_NUM] = {KEY_CACHE_INVALID, KEY_CACHE_INVALID, KEY_CACHE_INVALID};
    uint64_t keyL0CacheBIdx_[L0_BUF_NUM] = {KEY_CACHE_INVALID, KEY_CACHE_INVALID};
    uint64_t keyL0CacheS2_[L0_BUF_NUM] = {KEY_CACHE_INVALID, KEY_CACHE_INVALID};
...
__aicore__ inline void LightningIndexerServiceCube<LIT>::ComputeMm1(const LICommon::RunInfo &runInfo)
{
    ...
    for (uint64_t s2GmOffset = 0; s2GmOffset < s2ProcessSize; s2GmOffset += S2_BASIC_BLOCK) {
        ...
        // K 跨块缓存: tag 必须含 s2GmOffset(一个块内多个 s2 chunk 时各 chunk 的 K 不同)。
        // L0 槽按 kl0BufIdx_ 轮换(块粒度); L1 槽按 keyL1BufIdx_ 轮换(chunk 粒度)。
        uint64_t kl0Slot = kl0BufIdx_ % L0_BUF_NUM;
        uint64_t keyTag = s2GmBaseOffset + s2GmOffset;
        bool keyL0Hit = (keyL0CacheBIdx_[kl0Slot] == runInfo.tensorKeyOffset) &&
                        (keyL0CacheS2_[kl0Slot] == keyTag) && (s2ProcessSize > 0);
        if (!keyL0Hit) {
            // L1 K 槽缓存: 命中则跳过 GM→L1 的 Nd2Nz(同 batch 的 gS1 块重复搬运同一 K)
            uint64_t l1Slot = keyL1BufIdx_ % KEY_BUF_NUM;
            bool keyL1Hit = (keyL1CacheBIdx_[l1Slot] == runInfo.tensorKeyOffset) &&
                            (keyL1CacheS2_[l1Slot] == keyTag);
            if (!keyL1Hit) {
                if (PAGE_ATTENTION) {
                ...
                keyL1CacheBIdx_[l1Slot] = runInfo.tensorKeyOffset;
                keyL1CacheS2_[l1Slot] = keyTag;
        ...
        for (uint64_t s1gGmOffset = 0; s1gGmOffset < s1gProcessSize; s1gGmOffset += constInfo_.mBaseSize) {
            ...
            for (uint64_t s2L1Offset = 0; s2L1Offset < s2L1RealSize; s2L1Offset += S2_BASIC_BLOCK_L0) {
                ...
                for (uint64_t s1gL1Offset = 0; s1gL1Offset < s1gL1SizeAlign2G; s1gL1Offset += l0Stride) {
                    ...
                    if (s1gL1Offset == 0) {
                        // L0 K 槽命中: 同 batch 上一块已装载相同 K, 跳过 L1→L0 搬运
                        if (!keyL0Hit) {
                            LoadKeyToL0b(s2L1Offset, s2L1RealSize, s2L0RealSize, runInfo);
                            keyL0CacheBIdx_[kl0Slot] = runInfo.tensorKeyOffset;
                            keyL0CacheS2_[kl0Slot] = keyTag;
                        }
                    }
    ...
    CrossCoreSetFlag<LICommon::ConstInfo::LI_SYNC_MODE4, PIPE_FIX>(
        LICommon::ConstInfo::CROSS_CV_EVENT + runInfo.loop % 2 + LICommon::ConstInfo::AIV0_AIV1_OFFSET);
}
