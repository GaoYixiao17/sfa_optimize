...
| lightning_indexer(_v2)_service_vector_arch35.h 报 "no matching function for call to CreateVecIndex/Duplicate" (172/177 行附近) | 优化代码的退化路径模板用了 `tensor + offset` 偏移首参, CANN 9.1.0 的 `operator+` 返回类型与这些 API 不匹配; 已改为 AscendC 惯用法 `tensor[offset]` (与基线/框架自带算子一致, 9.1.0/9.2.0 通用, 指令完全等价) — `git pull` 后重编 |
