...
[📄 查看源码](https://gitcode.com/cann/ops-transformer/tree/9.2.0/posembedding/rotary_position_embedding)
...
- **返回值：**
  ...
  <tbody>
    ...
    <tr>
      <td>ACLNN_ERR_PARAM_INVALID</td>
      <td>161002</td>
      <td>传入的rotate为有效Tensor（该平台不支持rotate参数）。</td>
    </tr>
    <!-- end id16 -->
    <tr>
...
- <term>Ascend 950PR&950DT系列产品</term>：
  ...
  int main() {
      ...
      uint64_t workspaceSize = 0;
      aclOpExecutor* executor;
      // 调用aclnnRotaryPositionEmbeddingV2第一段接口，第5个入参传nullptr
      ret = aclnnRotaryPositionEmbeddingV2GetWorkspaceSize(x, cos, sin, mode, nullptr, out, & workspaceSize, &executor);
      CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnRotaryPositionEmbeddingV2GetWorkspaceSize   failed. ERROR: %d\n", ret); return ret);
      // 根据第一段接口计算出的workspaceSize申请device内存
      void* workspaceAddr = nullptr;
      if (workspaceSize > 0) {
