...
<!-- npu="950" id7 -->
> **自动微分说明**：仅支持<term>Ascend 950PR&950DT系列产品</term>。当`query`、`key`、`cos`、`sin`中任一输入`requires_grad=True`且`rotary_mode="half"`时，该接口支持自动微分，对loss执行`.backward()`时自动调用`apply_rotary_pos_emb_grad`计算`query`、`key`、`cos`、`sin`四路梯度。`rotary_mode`为`"quarter"`或`"interleave"`时不支持自动微分。
<!-- end id7 -->
...
<!-- end id9 -->
<!-- npu="310p" id10 -->
- <term>Atlas推理系列产品</term>：不支持`bfloat16`。
<!-- end id10 -->
...
- 单算子模式调用
    ...
    ```
<!-- npu="950" id11 -->
- <term>Ascend 950PR&950DT系列产品</term>训练模式调用（自动微分）
