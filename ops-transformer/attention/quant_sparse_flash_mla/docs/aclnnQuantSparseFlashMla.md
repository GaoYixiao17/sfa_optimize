...
- 接口功能：
  ...
  `aclnnQuantSparseFlashMla`算子旨在完成全量化和稀疏场景下的MLA（Multi-head Latent Attention）注意力计算。该接口支持以下三类计算模式：
  ...
  `aclnnQuantSparseFlashMlaMetadata`是`aclnnQuantSparseFlashMla`的分核信息，在主算子执行前生成。当前版本主算子必须传入该metadata。典型调用流程如下：

  **该算子不建议单独使用，建议与aclnnQuantSparseFlashMlaMetadata算子配合使用，形成完整的工作流。**

  1. 根据调用场景准备对应输入。
  2. 调用`aclnnQuantSparseFlashMlaMetadata`生成`metadata`，作为`aclnnQuantSparseFlashMla`的入参。
  3. 调用`aclnnQuantSparseFlashMla`，生成计算结果。

- 计算公式：

  $$
  O = \text{softmax}(Q \cdot \tilde{K}^T \cdot \text{softmaxScale}) \cdot \tilde{V}
  $$

  其中$\tilde{K} = \tilde{V}$（共享KV），$\tilde{K}$由滑动窗口内的原始KV和因果边界内的压缩KV拼接而成，具体参与计算的KV范围由模板模式和mask参数决定：

  - 滑动窗口部分（oriKv）：对第$i_{S1}$个Query token，其因果对角线位置为$\text{oriThreshold} = S2_{act} - S1_{act} + i_{S1} + 1$，窗口范围为$[\max(\text{oriThreshold} - \text{oriWinLeft} - 1, 0), \text{ori\_threshold} + \text{ori\_win\_right})$。
  ...
  注意力计算采用Online Softmax（Flash Attention V2），S2方向按分块循环，sinks作为每行softmax的初始最大值：
...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用`aclnnQuantSparseFlashMlaGetWorkspaceSize`接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用`aclnnQuantSparseFlashMla`执行实际计算。

```Cpp
aclnnStatus aclnnQuantSparseFlashMlaGetWorkspaceSize(
    const aclTensor *q,
    const aclTensor *oriKvOptional,
    const aclTensor *cmpKvOptional,
    const aclTensor *qDescaleOptional,
    const aclTensor *oriKvDescaleOptional,
    const aclTensor *cmpKvDescaleOptional,
...
```Cpp
aclnnStatus aclnnQuantSparseFlashMla(
    void          *workspace,
    uint64_t       workspaceSize,
    aclOpExecutor *executor,
    aclrtStream    stream)
...
## aclnnQuantSparseFlashMlaGetWorkspaceSize
...
- **参数说明**
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>当前版本必传。对应公式中K和V的一部分，表示原始不经压缩的量化KV。</td>
      <td>HIFLOAT8</td>
      <td>ND</td>
      <td>
        <ul>
    ...
    <tr>
      ...
      <td>可选输入。对应公式中K和V的一部分，表示经过压缩的量化KV。</td>
      <td>HIFLOAT8</td>
      <td>ND</td>
      <td>
        <ul>
    ...
    <tr>
    <td>qDescaleOptional（aclTensor*）</td>
      <td>输入</td>
      <td>对应q的量化参数。</td>
      <td>当前必传。</td>
      <td>FLOAT32</td>
    ...
    <tr>
      ...
      <td>传入cmpKvOptional场景且layoutKv为PA_BBND时必须传入。</td>
      <td>INT32</td>
      <td>ND</td>
    ...
    <tr>
      ...
      <td>输入</td>
      <td>AICPU算子aclnnQuantSparseFlashMlaMetadata的分核结果。</td>
      <td>必须传入。由aclnnQuantSparseFlashMlaMetadata算子生成。</td>
      <td>INT32</td>
      <td>ND</td>
      <td>(1024,)</td>
      <td>√</td>
    ...
    <tr>
      ...
      <td>输入</td>
      <td>表示量化模式。</td>
      <td>量化模式1表示Q、K、V 为per-token量化，数据类型为HIFLOAT8。</td>
      <td>-</td>
      <td>-</td>
      <td>-</td>
      <td>-</td>
    ...
    <tr>
      ...
      <td>支持：<br/>0: No Mask。<br/>3: RightDownCausal模式。<br/>4: Band模式。</td>
      <td>-</td>
      <td>-</td>
      <td>-</td>
      <td>-</td>
    ...
    <tr>
      ...
      <td>支持：<br/>0: No Mask。<br/>3: RightDownCausal模式。</td>
      <td>-</td>
      <td>-</td>
      <td>-</td>
      <td>-</td>
    ...
    <tr>
      ...
      <td>
        <ul>
          ...
          <li>layoutQ为TND时：(kvN, qS, qN/kvN)</li>
...
- **参数说明**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnQuantSparseFlashMlaGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
- 确定性计算
  ...
  - aclnnQuantSparseFlashMla默认采用确定性实现，相同输入多次调用结果一致。
...
|      特性参数组      |     参数字段名称     |
| :-------------------: | :-------------------: |
|      公共参数组      | q、quantMode、oriKvOptional、cmpKvOptional、qDescaleOptional、oriKvDescaleOptional、cmpKvDescaleOptional、metadataOptional、softmaxScale、layoutQ、layoutKv、attnOutOut |
...
<tbody>
    <tr>
        ...
        <td rowspan="4">
            <ul>
                ...
                <li>quantMode=1时，q、oriKvOptional和cmpKvOptional的dtype为HIFLOAT8</li>
            </ul>
        </td>
        <td rowspan="4">
            轴校验：
    ...
    <tr>
        ...
        <td>
            <ul>
                <li>dtype支持HIFLOAT8</li>
                <li>layoutKv为BSND时，oriKv的shape为(b, oriKvS, kvN, kvD)</li>
                <li>layoutKv为TND时，oriKv的shape为(oriKvT, kvN, kvD)</li>
                <li>layoutKv为PA_BBND时，oriKv的shape为(oriKvBlockNums, oriKvBlockSize, kvN, kvD)</li>
            </ul>
        </td>
        <td>
            当前版本必传
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <ul>
                    <li>dtype仅支持INT32</li>
                    <li>shape为(b, cmpKvSMax/cmpKvBlockSize)</li>
                    <li>值只能为正整数</li>
                </ul>
            </td>
            <td>
            ...
            <td>
                <ul>
                    <li>cmpBlockTable存在时，必须传入sequsedCmpKvOptional</li>
                    <li>PagedAttention开启情况下，blockTable必须不为空</li>
                </ul>
            </td>
        </tr>
        ...
        <tr>
            ...
            <td rowspan="2">
                <ul>
                    ...
                    <li>当returnSoftmaxLse为true时，softmaxLse的shape与layoutQ的关系如下：<ul><li>layoutQ为BSND时，softmaxLse的shape为(b, kvN, qS, qN/kvN)</li><li>layoutQ为TND时，softmaxLse的shape为(kvN, qT, qN/kvN)</li></ul></li>
                </ul>
            </td>
            <td>无</td>
        </tr>
