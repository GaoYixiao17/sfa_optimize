...
| 枚举名                                                           |  值  | 含义      |
| :------------------------------------------------------------ | :-: | :------ |
| `A8C8_QKV_HIF8_PER_TENSOR_P_FP8_E4M3_PER_TENSOR_SOFTMAX_FP32` |  0  | HIF8场景  |
| `A8C8_QKV_MXFP8_P_FP8_E4M3_PER_TENSOR_SOFTMAX_FP32`           |  1  | MxFP8场景 |
| `A8C8_QK_FP8_E4M3_PER_TOKEN_HEAD_V_FP8_E4M3_PER_HEAD_P_FP8_E4M3_PER_TENSOR_SOFTMAX_FP32` |  6  | FP8场景  |
...
> [!NOTE]
>
> 当前版本仅支持 mask_mode = 0（`NO_MASK`）和 3（`CAUSAL`）。
...
| seqused_q         | Tensor        | 可选    | 指定q中每batch实际使用的序列长度，截断冗余运算                                                          | int32  | ND   | (B,)   | ×         |
| seqused_kv        | Tensor        | 可选    | 指定kv中每batch实际使用的序列长度，截断冗余运算                                                         | int32  | ND   | (B,)   | ×         |
...
| head_dim_v       | int           | 可选    | v的每个注意力头维度，默认值为None。当前仅支持传None或与head_dim相等的值，传入其他值会拦截                          | int32  | -    | -      | -         |
...
| k                    | Tensor        | 必选    | 公式中的K                                                | float8_e4m3fn/hifloat8 | ND   | <ul><li>(KV_T, KV_N, D)</li><li>(Bn, KV_N, Bs, D)</li><li>(Bn, KV_N, D/32, Bs, 32)</li></ul>                   | √         |
| v                    | Tensor        | 必选    | 公式中的V                                                | float8_e4m3fn/hifloat8 | ND   | <ul><li>(KV_T, KV_N, D)</li><li>(Bn, KV_N, Bs, D)</li><li>(Bn, KV_N, D/32, Bs, 32)</li></ul>                   | √         |
...
| win_left            | int           | 可选    | window左界限                                            | int32                   | -    | -                                                                                                                  | -         |
| win_right           | int           | 可选    | window右界限                                            | int32                   | -    | -                                                                                                                  | -         |
| max_seqlen_q       | int           | 可选    | 指定查询q序列的长度上限，传入时必须大于等于0，-1表示未传入                  | int32                   | -    | -                                                                                                                  | -         |
| max_seqlen_kv      | int           | 可选    | 指定键k和值v序列的长度上限，传入时必须大于等于0，-1表示未传入                  | int32                   | -    | -                                                                                                                  | -         |
| layout_q            | string        | 可选    | 定义输入q张量的布局格式                                         | string                  | -    | -                                                                                                                  | -         |
| layout_q_descale   | string        | 可选    | 定义输入q_descale张量的布局格式                                | string                  | -    | -                                                                                                                  | -         |
| layout_kv           | string        | 可选    | 定义输入k和v张量的布局格式                                       | string                  | -    | -                                                                                                                  | -         |
| layout_out          | string        | 可选    | 定义输出张量的布局格式                                          | string                  | -    | -                                                                                                                  | -         |
...
| 参数名      | 参数类型   | 可选/必选 | 描述                        | 数据类型  | 数据格式 | 维度                       | 非连续Tensor |
| :------- | :----- | :---- | :------------------------ | :---- | :--- | :----------------------- | :-------- |
| metadata | Tensor | 必选    | quant_flash_attn的任务切分数据 | int32 | ND   | (2, max_schedule_size) | ×         |

### quant_flash_attn

| 参数名          | 参数类型   | 可选/必选 | 描述                                                                                                           | 数据类型     | 数据格式 | 维度              | 非连续Tensor |
| :----------- | :----- | :---- | :----------------------------------------------------------------------------------------------------------- | :------- | :--- | :-------------- | :-------- |
| attn_out    | Tensor | 必选    | quant_flash_attn的计算输出                                                                                     | bfloat16 | ND   | <ul><li>(Q_T, Q_N, D)</li><li>(B, Q_S, Q_N, D)（HIF8场景）</li><li>(B, Q_N, Q_S, D)（HIF8场景）</li></ul> | ×         |
...
|       <br />       |           k          |       INPUT      | Tensor |
|       <br />       |           v          |       INPUT      | Tensor |
|       <br />       |       metadata       |  INPUT(OPTIONAL) | Tensor |
|       <br />       |    softmax_scale    |  ATTR(OPTIONAL)  | double |
|       <br />       |       layout_q      |  ATTR(OPTIONAL)  | string |
|       <br />       |  layout_q_descale  |  ATTR(OPTIONAL)  | string |
|       <br />       |      layout_kv      |  ATTR(OPTIONAL)  | string |
|       <br />       |      layout_out     |  ATTR(OPTIONAL)  | string |
|       <br />       |       attn_out      |      OUTPUT      | Tensor |
|       全量化参数组       |      quant_mode     |       ATTR       |   int  |
|       <br />       |      q_descale      |       INPUT      | Tensor |
|       <br />       |      k_descale      |       INPUT      | Tensor |
|       <br />       |      v_descale      |       INPUT      | Tensor |
|       <br />       |       p_scale       |  INPUT(OPTIONAL) | Tensor |
|       Mask参数组      |      mask_mode      |  ATTR(OPTIONAL)  |   int  |
|       <br />       |       win_left      |  ATTR(OPTIONAL)  |   int  |
|       <br />       |      win_right      |  ATTR(OPTIONAL)  |   int  |
|       <br />       |      attn_mask      |  INPUT(OPTIONAL) | Tensor |
|     SeqLengths参数组     |    cu_seqlens_q    |  INPUT(OPTIONAL) | Tensor |
|       <br />       |    cu_seqlens_kv   |  INPUT(OPTIONAL) | Tensor |
|       <br />       |      seqused_q      |  INPUT(OPTIONAL) | Tensor |
|       <br />       |      seqused_kv     |  INPUT(OPTIONAL) | Tensor |
...
<ul>
    <li>quant_mode=0，A8C8_QKV_HIF8_PER_TENSOR_P_FP8_E4M3_PER_TENSOR_SOFTMAX_FP32（HIF8场景）</li>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <ul>
                    ...
                    <li>shape dim：MxFP8场景支持4、5；FP8场景支持2；HIF8场景支持1</li>
                </ul>
            </td>
        ...
        <tr>
            ...
            <td>
                <ul>
                    <li>tensor_type支持float8_e8m0、float32</li>
                    <li>shape dim：MxFP8场景支持4、5、6；FP8场景支持3；HIF8场景支持1</li>
                </ul>
            </td>
        </tr>
        <tr>
            ...
            <td>
                <ul>
                    <li>tensor_type支持float8_e8m0、float32</li>
                    <li>shape dim：MxFP8场景支持4、5、6；FP8场景支持1；HIF8场景支持1</li>
                </ul>
            </td>
        </tr>
        <tr>
...
> [!NOTE]
>
> MxFP8场景下，v_descale在TND（非Paged Attention）场景下，shape中第一维KV_T/64并非简单整数除法，而是各batch实际KV序列长度按64向上取整后的累加和，即 Σ ceil(cu_seqlens_kv[b+1] - cu_seqlens_kv[b], 64)，其中cu_seqlens_kv为累积序列长度。因此各batch的KV序列长度无需严格64对齐。

q/k/v/attn_out shape匹配关系表：<a name="qkv_attn_out_shape匹配关系表"></a>
...
<table style="undefined;table-layout: fixed; width:1625px">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>
                <ul>
                    <li>可选属性，默认值为-1，表示无穷（极大值），当前版本需保持默认值</li>
                </ul>
            </td>
        </tr>
    </tbody>
...
> [!NOTE]
>
> 算子不校验 `seqused_q`/`seqused_kv` 中的最大值是否与 `max_seqlen_q`/`max_seqlen_kv` 一致。若同时传入，用户需自行保证 `max(seqused_q) <= max_seqlen_q`、`max(seqused_kv) <= max_seqlen_kv`。
...
<table style="undefined;table-layout: fixed; width:1625px">
    ...
    <tbody>
        <tr>
            ...
            <td>
                <ul>
                    <li>当layout_kv为PA场景时，必须传入seqused_kv</li>
                    <li>当layout_kv为PA场景时，block_table必须不为空</li>
                    <li>Paged Attention场景下k/v/k_descale/v_descale支持非连续Tensor，支持的轴非连续与layout_kv相关：layout_kv为PA_BNBD/PA_NZ时，支持0轴或0轴1轴非连续；layout_kv为PA_BBND时，仅支持0轴非连续</li>
                    <li>块大小仅MxFP8场景支持，取值为64、128、256、512或1024</li>
...
- quant_flash_attn_metadata + quant_flash_attn联合调用示例（TND，非PA场景）
  ...
  # v_descale第一维为各batch序列长度按64向上取整后求和
  v_descale = torch.randn(B * ((KV_S + 63) // 64), KV_N, D, 2, dtype=torch.float8_e8m0, device="npu")
