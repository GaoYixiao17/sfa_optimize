...
1. 根据产品形态划分的通用能力模板：
（1）A2/A3场景通用能力模板，支持<term>Atlas A2系列产品</term>和<term>Atlas A3系列产品</term>。
(2)针对Atlas推理系列产品，开启通用能力模板。
...
- C侧总耗时 > V侧总耗时
  ...
  该场景流水特征下，Vector计算节点少，计算速度快，在<term>Atlas A2系列产品</term> C:V=1:2的情况下，Cube的搬运时长足以掩盖Vector的计算时长，因此只要关注Cube的MTE2耗时即可，最终达成MTE2 bound。在Cube双发机制下，提前发射两块Cube计算，Cube1、Cube2计算可以衔接，使得Cube利用率最高，达成Cube bound。
