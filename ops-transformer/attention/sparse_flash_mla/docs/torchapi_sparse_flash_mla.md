...
| layout_q | layout_kv | q/kv维度 | softmax_lse维度（开启时） |
| :--- | :--- | :--- | :--- |
...
| cmp_ratio | int32；SWA场景取值为1，CSA/HCA场景取值范围1-128。 | 可选，默认1。 | 必须同时与`metadata`、`cmp_kv`长度和`cmp_residual_kv`一致。 | <term>Atlas A3系列产品</term>：SWA取非1、CSA取非4、HCA取非128时拦截；<term>Ascend 950PR&950DT系列产品</term>：SWA取非1或CSA/HCA取非1-128时拦截。 |
