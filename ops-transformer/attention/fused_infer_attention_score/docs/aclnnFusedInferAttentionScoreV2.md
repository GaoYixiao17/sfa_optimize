...
算子执行接口为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnFusedInferAttentionScoreV2GetWorkspaceSize”接口获取入参并根据计算流程计算所需workspace大小，再调用“aclnnFusedInferAttentionScoreV2”接口执行计算。
...
- 参数key、value使用限制：
  ...
  <!-- npu="910b" id6 -->
  - <term>Atlas A2系列产品</term>：数据类型支持FLOAT16、BFLOAT16、INT8、INT4(INT32)。
  <!-- end id6 -->
  <!-- npu="950" id7 -->
  - <term>Ascend 950PR&950DT系列产品</term>：数据类型支持FLOAT16、BFLOAT16、INT8、HIFLOAT8、FLOAT8_E4M3FN、INT4(INT32)、FLOAT4_E2M1。
...
- 伪量化参数antiquantScale和antiquantOffset约束：
  ...
  <!-- npu="910b" id8 -->
  - <term>Atlas A2系列产品</term>：Q_S等于1时，数据类型支持FLOAT16、BFLOAT16、FLOAT32。Q_S大于等于2时只支持FLOAT16。
  <!-- end id8 -->
  <!-- npu="950" id9 -->
  - <term>Ascend 950PR&950DT系列产品</term>：数据类型支持FLOAT16、BFLOAT16、FLOAT32。
...
- keyAntiquantScaleOptional：Device侧的aclTensor，数据格式支持ND，kv伪量化参数分离时表示key的反量化因子。如不使用该功能时可传入nullptr。
  ...
  <!-- npu="910b" id10 -->
  - <term>Atlas A2系列产品</term>：数据类型支持FLOAT16、BFLOAT16、FLOAT32。Q_S大于等于2时仅支持per-token和per-channel模式。Q_S等于1时仅支持per-tensor，per-channel，per-token，per-tensor叠加per-head，per-token叠加per-head，per-token叠加使用page attention模式管理scale/offset、per-token叠加per head并使用page attention模式管理scale/offset。
  <!-- end id10 -->
  <!-- npu="950" id11 -->
  - <term>Ascend 950PR&950DT系列产品</term>：数据类型支持FLOAT16、BFLOAT16、FLOAT32、FLOAT8_E8M0。支持per-tensor，per-channel，per-token，per-tensor叠加per-head，per-token叠加per-head，per-token叠加使用page attention模式管理scale/offset、per-token叠加per head并使用page attention模式管理scale/offset和per-token-group。
...
- keyAntiquantOffsetOptional：Device侧的aclTensor，数据类型支持FLOAT16、BFLOAT16、FLOAT32。数据格式支持ND，kv伪量化参数分离时表示key的反量化偏移。如果使用该功能其数据类型与shape必须与keyAntiquantScaleOptional保持一致。如不使用该功能时可传入nullptr。
  ...
  <!-- npu="910b" id12 -->
  - <term>Atlas A2系列产品</term>：Q_S大于等于2时仅支持per-token和per-channel模式。Q_S等于1时仅支持per-tensor，per-channel，per-token，per-tensor叠加per-head，per-token叠加per-head，per-token叠加使用page attention模式管理scale/offset、per-token叠加per head并使用page attention模式管理scale/offset。
  <!-- end id12 -->
  <!-- npu="950" id13 -->
  - <term>Ascend 950PR&950DT系列产品</term>：支持per-tensor，per-channel，per-token，per-tensor叠加per-head，per-token叠加per-head，per-token叠加使用page attention模式管理scale/offset、per-token叠加per head并使用page attention模式管理scale/offset。
...
- inputLayout：用于标识输入query、key、value的数据排布格式，当前支持BSH、BSND、BNSD、BNSD_BSND(输入为BNSD时，输出格式为BSND，仅支持Q_S大于1)、TND。用户不特意指定时建议传入"BSH"。
        ...
        <tbody>
            ...
            <tr>
                <td colspan="3" style="text-align: left; ">
                    <strong>特殊约束：</strong>Q_S为1时，sparseMode参数无效。
                </td>
            </tr>
          </tbody>
      </table>
...
- keyAntiquantMode使用限制如下：
  ...
  <!-- end id18 -->
  <!-- npu="950" id19 -->
  - Ascend 950PR&950DT系列产品：传入0，1，2，3，4，5和6之外的其他值会执行异常。
...
- **当Q_S大于1时**：
  ...
  - page attention场景：
    ...
    - page attention的开启必要条件是blockTable存在且有效，同时key、value是按照blockTable中的索引在一片连续内存中排布，在该场景下key、value的inputLayout参数无效。blockTable中填充的是blockid，当前不会对blockid的合法性进行校验，需用户自行保证。
      ...
      <!-- npu="910b" id25 -->
      - <term>Atlas A2系列产品</term>：支持key、value dtype为FLOAT16/BFLOAT16。
      <!-- end id25 -->
      <!-- npu="950" id26 -->
    ...
    - page attention伪量化场景
      ...
      <!-- end id27 -->
      <!-- npu="950" id28 -->
      - <term>Ascend 950PR&950DT系列产品</term>：支持query dtype为FLOAT16/BFLOAT16，支持key、value dtype为INT8/HIFLOAT8/FLOAT8_E4M3FN/FLOAT4_E2M1/INT4(INT32)。
      <!-- end id28 -->
    ...
    - page attention全量化场景
      ...
      <!-- end id29 -->
      <!-- npu="950" id30 -->
      - <term>Ascend 950PR&950DT系列产品</term>：支持query和kv cache全部为INT8。
      <!-- end id30 -->
    ...
    - page attention的开启场景下，以下场景输入KV_S需要大于等于maxBlockNumPerSeq * blockSize
      - 传入attenMask时，例如mask shape为(B, 1, Q_S, KV_S)
      - 传入pseShift时，例如pseShift shape为(B, N, Q_S, KV_S)
      - 开启伪量化per-token模式：输入参数antiquantScale和antiquantOffset的shape均为(2,B,S)
      - 开启伪量化per-token叠加per-head模式：输入参数antiquantScale和antiquantOffset的shape均为(B,N,S),数据类型固定为FLOAT32，当key,value数据类型为INT8，INT4(INT32)时支持。
      - 开启per-token-group模式：antiquantScale的shape为\(1, B, N, S, D/32\),数据类型固定为FLOAT8_E8M0，不支持带antiquantOffset。当key、value数据类型为FLOAT4_E2M1时支持。
  - query左padding场景：
    ...
    - 需要与actualSeqLengths参数一起开启，否则默认为query右padding场景。
  ...
  - 输出为INT8时，入参quantOffset2传入非空指针和非空tensor值，并且sparseMode、preTokens和nextTokens满足以下条件，矩阵会存在某几行不参与计算的情况，导致计算结果误差，该场景会拦截（解决方案：如果希望该场景不被拦截，需要在FIA接口外部做后量化操作，不在FIA接口内部开启）：
    ...
    - sparseMode = 0，attenMask如果非空指针，每个batch actualSeqLengths - actualSeqLengthsKV - actualSharedPrefixLen - preTokens > 0或nextTokens < 0时，满足拦截条件
    - sparseMode = 1或2，不会出现满足拦截条件的情况
    - sparseMode = 3，每个batch actualSeqLengthsKV + actualSharedPrefixLen - actualSeqLengths < 0，满足拦截条件
    - sparseMode = 4，preTokens < 0或每个batch nextTokens + actualSeqLengthsKV + actualSharedPrefixLen - actualSeqLengths < 0时，满足拦截条件
  ...
  - pseShift功能使用限制如下：
    ...
    <!-- npu="950" id33 -->
    - <term>Ascend 950PR&950DT系列产品</term>：非量化，全量化场景：无对齐限制。
    <!-- end id33 -->
...
- **当Q_S等于1时**：
  ...
  - page attention场景：
    ...
    - page attention开启场景下，当输入kv cache排布格式为BnBsH，且numKvHeads * headDim超过64k时，受硬件指令约束，会被拦截报错。可通过开启GQA（减小numKvHeads）或调整kv cache排布格式为BnNBsD解决。
    - page attention不支持tensorlist场景，不支持左padding场景。
    - page attention场景下，必须传入actualSeqLengthsKv。
    ...
    - page attention的开启场景下，以下场景输入S需要大于等于maxBlockNumPerSeq * blockSize。
      - 开启Attention mask，如mask shape为 \(B, 1, 1, S\)。
      - 开启pseShift，如pseShift shape为\(B, N, 1, S\)。
      - 开启伪量化per-token模式：输入参数antiquantScale和antiquantOffset的shape均为\(2, B, S\)。
