# aclnnInplaceFusedCausalConv1dV2

[📄 查看源码](https://gitcode.com/cann/ops-transformer/tree/9.2.0/attention/inplace_fused_causal_conv1d)
...
- 接口功能：对序列执行因果一维卷积，沿序列维度使用缓存数据（长度为卷积核宽减1）对各序列头部进行padding，确保输出依赖当前及历史输入；卷积完成后，将当前序列部分数据更新到缓存；在因果一维卷积输出的基础上，将原始输入加到输出上以实现残差连接。支持APC（Automatic Prefix Caching）、MTP（Multi-Token Prediction，多令牌预测，可用于投机解码）、残差连接等特性。相较于aclnnInplaceFusedCausalConv1dV1，本接口新增支持maxDraftTokens参数，表示最大支持的投机个数。相较于FusedCausalConv1d算子，支持原地更新。<br>
...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用`aclnnInplaceFusedCausalConv1dV2GetWorkspaceSize`接口获取入参并计算所需workspace大小以及包含了算子计算流程的执行器，再调用`aclnnInplaceFusedCausalConv1dV2`接口执行计算。

```Cpp
aclnnStatus aclnnInplaceFusedCausalConv1dV2GetWorkspaceSize(
  aclTensor       *x,
...
```Cpp
aclnnStatus aclnnInplaceFusedCausalConv1dV2(
  void          *workspace,
  uint64_t       workspaceSize,
  aclOpExecutor *executor,
  aclrtStream    stream)
...
## aclnnInplaceFusedCausalConv1dV2GetWorkspaceSize
...
- **参数说明**：
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><ul><li>不支持空tensor。</li><li>shape为[..., stateLen, dim]，第0维的大小不固定。</li><li>stateLen == K-1+m，prefill场景下m=0，decode、PD混部场景下m=[0, maxDraftTokens]。</li><li>m表示投机个数，映射numAcceptedTokens参数的值。</li><li>支持dim0和dim1非连续且stride大于等于1</li></ul></td>
      <td>数据类型与x一致</td>
      <td>ND</td>
      <td>3</td>
      <td>√</td>
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnInplaceFusedCausalConv1dV2GetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
- 确定性计算：
  - aclnnInplaceFusedCausalConv1dV2默认确定性实现。
...
#include <cmath>
#include "acl/acl.h"
#include "aclnnop/aclnn_inplace_fused_causal_conv1d_v2.h"
...
int main()
{
    ...
    ret = aclnnInplaceFusedCausalConv1dV2GetWorkspaceSize(
        ...
        activationMode, padSlotId, runMode, maxQueryLen, residualConnection, blockSize, convMode, maxDraftTokens, &workspaceSize,
        &executor);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnInplaceFusedCausalConv1dV2GetWorkspaceSize failed. ERROR: %d\n", ret);
              return ret);
    ...
    ret = aclnnInplaceFusedCausalConv1dV2(workspaceAddr, workspaceSize, executor, stream);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnInplaceFusedCausalConv1dV2 failed. ERROR: %d\n", ret); return ret);
