...
- 相关约束：
  ...
  - <term>Ascend 950PR&950DT系列产品</term>：
    - `topk_weights`可传有效Tensor或None。传有效Tensor时开启topK专家权重功能，有效Tensor为2D Tensor，shape为(BS, K)，且`expand_scales`输出有效；传None时不开启topK专家权重功能，`expand_scales`输出无效。不支持空Tensor。
