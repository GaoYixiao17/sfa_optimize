...
[📄 查看源码](https://gitcode.com/cann/ops-transformer/tree/9.2.0/mc2/allto_all_matmul)
...
- 新增`commMode`参数，用户根据该参数指定芯片使用的通信引擎。
  ...
  <!-- end id7 -->
  <!-- npu="A3" id8 -->
  - <term>Atlas A3系列产品</term>：无新增特性，`commMode`仅支持`ai_cpu`。
  <!-- end id8 -->
  <!-- npu="910b" id9 -->
...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnAlltoAllMatmulV2GetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnAlltoAllMatmulV2”接口执行计算。
...
* H*rankSize范围，根据设备型号有不同限制：
  ...
  <!-- npu="910b" id13 -->
  - <term>Atlas A2系列产品</term>：支持[1, 35000]。
  <!-- end id13 -->
  <!-- npu="950,A3" id14 -->
  - <term>Atlas A3系列产品</term>  、<term>Ascend 950PR&950DT系列产品</term>：支持[2, 65535]。
  <!-- end id14 -->
...
* 空tensor的支持度根据不同设备型号有不同的限制：
  ...
  <!-- end id15 -->
  <!-- npu="950,A3" id16 -->
  - <term>Atlas A3系列产品</term>  、<term>Ascend 950PR&950DT系列产品</term>：仅支持输入x1的第一维度（BS）为0的空tensor，其它空tensor均不支持。
  <!-- end id16 -->

* 非连续tensor的支持度根据不同设备型号有不同的限制：

  <!-- npu="A3,910b" id17 -->
  - <term>Atlas A2系列产品</term>、<term>Atlas A3系列产品</term>  ：不支持任何非连续tensor。
  <!-- end id17 -->
  <!-- npu="950" id18 -->
...
* biasOptional的数据类型根据不同设备型号有不同的限制：
  ...
  <!-- npu="A3,910b" id19 -->
  - <term>Atlas A2系列产品</term>、<term>Atlas A3系列产品</term>  ：x1/x2计算输入的数据类型为FLOAT16时，biasOptional计算输入的数据类型支持FLOAT16；x1/x2计算输入的数据类型为BFLOAT16时，biasOptional计算输入的数据类型支持FLOAT32。
  <!-- end id19 -->
  <!-- npu="950" id20 -->
...
* 通信引擎参数commMode根据不同设备型号有不同的限制：
   ...
   <!-- end id21 -->
   <!-- npu="A3" id22 -->
   - <term>Atlas A3系列产品</term>：仅支持输入`ai_cpu`。
   <!-- end id22 -->
   <!-- npu="950" id23 -->
...
- <term>Atlas A2系列产品</term>：
    ...
    #include "aclnnop/aclnn_allto_all_matmul_v2.h"
