...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用`aclnnAlltoAllvGroupedMatMulGetWorkspaceSize`接口获取入参并根据计算流程计算所需workspace大小，再调用`aclnnAlltoAllvGroupedMatMul`接口执行计算。
...
- **参数说明**
    ...
    <tr>
        ...
        <td>数据类型支持INT64，长度为e * epWorldSize，最大为384。</td>
