...
- 新增`commMode`参数，用户根据该参数指定芯片使用的通信引擎。
  ...
  <!-- npu="A3" id7 -->
  - <term>Atlas A3系列产品</term>：支持`ai_cpu`和`aiv`。

  <!-- end id7 -->

  <!-- npu="910b" id8 -->
  - <term>Atlas A2系列产品</term>：仅支持`aiv`，不支持`ai_cpu`和`ccu`。
...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用`aclnnGroupedMatMulAlltoAllvV2GetWorkspaceSize`接口获取入参并根据计算流程计算所需workspace大小，再调用`aclnnGroupedMatMulAlltoAllvV2`接口执行计算。
...
- **参数说明**
    ...
    <td><ul><li>输入类型需为aclIntArray*，元素类型为INT64。</li><li>list大小为e * epWorldSize，AIV通信最大为1024，其他通信引擎最大为256。</li></ul></td>
