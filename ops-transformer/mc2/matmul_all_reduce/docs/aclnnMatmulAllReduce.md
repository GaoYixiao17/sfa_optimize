...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用`aclnnMatmulAllReduceGetWorkspaceSize`接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用`aclnnMatmulAllReduce`接口执行计算。
...
- 确定性计算：
  ...
  <!-- npu="910b" id7 -->
  - <term>Atlas A2系列产品</term>：`aclnnMatmulAllReduce`默认非确定性实现，支持通过配置`HCCL_DETERMINISTIC`环境变量为true开启确定性计算。
  <!-- end id7 -->
  <!-- npu="950" id8 -->
  - Ascend 950PR&950DT系列产品：`aclnnMatmulAllReduce`默认确定性实现。
  <!-- end id8 -->
