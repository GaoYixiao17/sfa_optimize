...
- **参数说明**
    ...
    - <term>Atlas A2系列产品</term>：
        ...
        - 输入x2的数据格式支持ND（当前版本仅支持二维输入）和`FRACTAL_NZ`格式（当前版本仅支持四维输入）。当x2的数据格式为`FRACTAL_NZ`时，配合`aclnnCalculateMatmulWeightSizeV2`和`aclnnTransMatmulWeight`完成数据格式ND到数据格式NZ的转换，非连续的tensor仅支持transpose场景。
...
- 确定性计算：
  ...
  <!-- npu="910b" id9 -->
  - <term>Atlas A2系列产品</term>：`aclnnQuantMatmulAllReduceV2`默认非确定性实现，支持通过配置`HCCL_DETERMINISTIC`环境变量为true开启确定性计算。
  <!-- end id9 -->
  <!-- npu="950" id10 -->
  - Ascend 950PR&950DT系列产品：`aclnnQuantMatmulAllReduceV2`默认确定性实现。
  <!-- end id10 -->
