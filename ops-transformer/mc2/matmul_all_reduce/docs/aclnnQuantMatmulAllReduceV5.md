...
- **接口功能**：兼容`aclnnQuantMatmulAllReduce`、`aclnnQuantMatmulAllReduceV2`、`aclnnQuantMatmulAllReduceV3`、`aclnnQuantMatmulAllReduceV4`支持的功能，新增通信引擎参数commMode，支持自选通信引擎。
- **计算公式**：与`aclnnQuantMatmulAllReduceV4`一致，详见[aclnnQuantMatmulAllReduceV4计算公式](aclnnQuantMatmulAllReduceV4.md#功能说明)。

## 函数原型

每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用`aclnnQuantMatmulAllReduceV5GetWorkspaceSize`接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用`aclnnQuantMatmulAllReduceV5`接口执行计算。
...
## aclnnQuantMatmulAllReduceV5GetWorkspaceSize
...
- **参数说明**
  ...
  <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口<code>aclnnQuantMatmulAllReduceV5GetWorkspaceSize</code>获取。</td>
  </tr>
  <tr>
      <td>executor</td>
      <td>输入</td>
...
- <term>Atlas A2系列产品</term>、<term>Ascend 950PR&950DT系列产品</term>：
  ...
  int launchOneThreadQuantMatmulAllReduce(Args &args) {
      ...
      LOG_PRINT("device%d aclnnQuantMatmulAllReduceV5 execute success \n", args.rankId);
