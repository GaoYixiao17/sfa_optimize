...
| cmp_ratio      | 可选属性      | 用于稀疏计算，表示key的压缩倍数。数据类型支持`int32`。Atlas A3推理系列产品支持1/2/4/8/16/32/64/128，Ascend 950PR&950DT系列产品支持1/4/128，默认值1。 | INT32          | -         |
...
- <term>Ascend 950PR&950DT系列产品</term>：query、key不支持INT8；weights、query_dequant_scale和key_dequant_scale不支持FLOAT16。
- <term>Atlas A3系列产品</term>、<term>Atlas A2系列产品</term>：query、key不支持FLOAT8_e4m3fn和HIFLOAT8；weights、query_dequant_scale和key_dequant_scale不支持FLOAT32。


## 约束说明
-   该接口支持图模式。
-   该接口要求$W \odot Scale_Q$的结果在`float16`(Atlas A3)/`float32`(Ascend 950PR&950DT系列产品)的表示范围内。
