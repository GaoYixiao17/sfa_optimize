...
- 若开发者不使用算子执行接口调用算子，也可以定义基于Ascend IR的算子描述文件，通过ATC工具编译获得算子om文件，然后加载模型文件执行算子，详细调用方法可参见[《应用开发指南》](https://hiascend.com/document/redirect/CannCommunityCppOpcall)中的单算子调用 > 单算子模型执行章节。
