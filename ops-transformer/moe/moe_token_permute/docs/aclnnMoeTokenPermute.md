...
[📄 查看源码](https://gitcode.com/cann/ops-transformer/tree/9.2.0/moe/moe_token_permute)
...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnMoeTokenPermuteGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnMoeTokenPermute”接口执行计算。
...
<!-- npu="950" id7 -->
- <term>Ascend 950PR&950DT系列产品</term>：仅该产品支持INT8类型的`tokens`和`permuteTokensOut`，按非量化方式处理，且二者数据类型必须一致；INT8时`tokens`维度必须为2，不支持3维及以上。
<!-- end id7 -->
<!-- npu="950" id8 -->
- <term>Ascend 950PR&950DT系列产品</term>：当`tokens`数据类型为INT8时，`indices`表示expert ID，取值范围为`[0, 10240)`，最大值为`10239`，不支持`10240`。
<!-- end id8 -->
...
int main() {
    ...
    ret = aclnnMoeTokenPermuteGetWorkspaceSize(x, indices, numTokenOut, padMode, expandedXOut,
