...
private:
    ...
    __aicore__ inline void StageSparseIdx(int64_t s2Base, const RunInfo &runInfo, ConstInfo &constInfo);
    ...
    // ============ 稀疏索引批量预取优化 ============
    // 原 ProcessSparseKv 每对 token 串行执行 4 次 GM 标量读(sparseIndices×2 + blockTable×2),
    // B=32/K=128 全网约 53 万次串行 load。 改为: 每 16 行批次一次 DataCopyPad 预取 16 个
    // sparse_indices 到 UB, 循环内改从 UB 读; blockTable 增加 1 项标量缓存(相邻选中 token
    // 大多落在同一 KV cache block, sparse_indices 无序但局部性好)。
    static constexpr int64_t SPARSE_IDX_STAGE_NUM = 16; // 内层循环每批最多 8 对 = 16 个索引
    TBuf<> sparseIdxStageBuf;
    LocalTensor<int32_t> sparseIdxStageUb;
    int64_t sparseIdxStagePosBase = 0; // 预取的第 0 个条目在 k 序列中的位置
    int32_t sparseIdxStageCount = 0;   // 预取的有效条目数
    int64_t btCacheKey = -1;           // blockTable 1 项缓存键: (boIdx<<32)|blkTableIdx
    int64_t btCacheVal = 0;            // 对应的 KV 块起始 token 偏移
};

TEMPLATES_DEF_NO_DEFAULT __aicore__ inline void SFAVectorService<TEMPLATE_ARGS>::StageSparseIdx(
    int64_t s2Base, const RunInfo &runInfo, ConstInfo &constInfo)
{
    ...
    int64_t base = runInfo.s2LoopCount * constInfo.s2BaseSize + s2Base;
    int64_t remain = Min(Min(sparseS2End - s2Base, SPARSE_IDX_STAGE_NUM), constInfo.sparseBlockCount - base);
    sparseIdxStagePosBase = base;
    sparseIdxStageCount = (remain > 0) ? static_cast<int32_t>(remain) : 0;
    if (sparseIdxStageCount <= 0) {
        return;
    }
    // DataCopy 要求 src 32B 对齐且长度为 32B 的倍数: 向下对齐拷贝起点, 窗口用行尾钳制
    // (绝不越过当前 (b,s1) 行 → 不会越界读张量); 行尾余量不足时回退为逐元素标量读
    // (与原实现同代价, 仅影响每行最后一批)
    int64_t pos = topkBS1Idx + base;
    int64_t rowEnd = topkBS1Idx + constInfo.sparseBlockCount;
    int64_t posAligned = pos & ~static_cast<int64_t>(7);
    int64_t need = sparseIdxStageCount + (pos - posAligned);
    int64_t copyLen = Min((need + 7) & ~static_cast<int64_t>(7), (rowEnd - posAligned) & ~static_cast<int64_t>(7));
    if (copyLen >= need) {
        DataCopy(sparseIdxStageUb, sparseIndicesGm[posAligned], static_cast<uint32_t>(copyLen));
        SetFlag<HardEvent::MTE2_V>(mte2ToV);
        WaitFlag<HardEvent::MTE2_V>(mte2ToV);
        sparseIdxStagePosBase = posAligned;
        sparseIdxStageCount = static_cast<int32_t>(copyLen);
    } else {
        for (int64_t k = 0; k < sparseIdxStageCount; k++) {
            sparseIdxStageUb.SetValue(static_cast<uint32_t>(pos - posAligned + k),
                                      sparseIndicesGm.GetValue(pos + k));
        }
        sparseIdxStagePosBase = posAligned;
        sparseIdxStageCount = static_cast<int32_t>(pos - posAligned + sparseIdxStageCount);
    }
}
    ...
    int64_t &token0Idx, int64_t &token1Idx, int64_t s2IdxInBase, const RunInfo &runInfo, ConstInfo &constInfo)
{
    ...
    int64_t relPos = topkKIdx - sparseIdxStagePosBase;
    if (unlikely(topkKIdx >= constInfo.sparseBlockCount) || relPos < 0 || relPos >= sparseIdxStageCount) {
        token0Idx = -1;
    } else {
        token0Idx = sparseIdxStageUb.GetValue(static_cast<uint32_t>(relPos)) + runInfo.s2StartIdx;
    }
    topkKIdx += 1;
    relPos += 1;
    if (unlikely((topkKIdx >= constInfo.sparseBlockCount) || (s2IdxInBase + 1 >= sparseS2End)) || relPos < 0 ||
        relPos >= sparseIdxStageCount) {
        token1Idx = -1;
    } else {
        token1Idx = sparseIdxStageUb.GetValue(static_cast<uint32_t>(relPos)) + runInfo.s2StartIdx;
    }
}
                                                                                                 ...
                                                                                                 ConstInfo &constInfo)
{
    ...
    if constexpr (isPa) {
        ...
        // 1 项标量缓存: 相邻选中 token 大多落在同一 KV cache block, 命中则免 GM 标量读
        int64_t btKey = (static_cast<int64_t>(runInfo.boIdx) << 32) + blkTableIdx;
        if (btKey != btCacheKey) {
            btCacheVal = blockTableGm.GetValue(runInfo.boIdx * maxBlockNumPerBatch + blkTableIdx);
            btCacheKey = btKey;
        }
        realkeyOffset = btCacheVal * constInfo.keyStride0 + blkTableOffset; // BlockNum, BlockSize, N(1), D
    } else {
    ...
    int32_t startPos)
{
    ...
    while ((s2 < sparseS2End) && !meetEnd) { // 拷贝到s2End或者遇到-1
        int64_t dealRow = 0;
        // 0、批量预取本批最多 16 个 sparse_indices 到 UB, 消除内层循环逐次 GM 标量读
        StageSparseIdx(s2, runInfo, constInfo);
        // 1、copy kv in, gm ->ub
                                                                                                 ...
                                                                                                 ConstInfo &constInfo)
{
    ...
    // 稀疏索引批量预取缓冲: 最多 16 个索引 + 32B 对齐窗口的 7 个头部冗余
    tPipe->InitBuffer(sparseIdxStageBuf, (SPARSE_IDX_STAGE_NUM + 8) * sizeof(int32_t));
    sparseIdxStageUb = sparseIdxStageBuf.Get<int32_t>();
    btCacheKey = -1;
